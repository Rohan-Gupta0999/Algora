...
const ProposePanel = (() => {
        ...
        <div class="pp-body">
          ...
          <div class="pp-section">
            ...
            <div class="pp-add-row">
              <input class="pp-input" id="pp-cosigner-input" type="email" placeholder="co-signatory@gov.in" style="flex:1;">
  ...
  return {
    ...
    _addCoSigner() {
      ...
      if (!email || !email.includes('@') || coSigners.includes(email)) return;
      if (window.currentUser && email === window.currentUser.email) return;
      coSigners.push(email); input.value = ''; _renderCoSigners();
    },
    ...
    async _submit() {
      ...
      try {
        if (typeof db !== 'undefined') await db.collection('transactions').doc(newId).set(txData);
        if (typeof window.onProposeTxSubmit === 'function') window.onProposeTxSubmit(txData);
        ProposePanel.close();
        if (typeof showToast === 'function') showToast('📤 ' + newId + ' proposed — emails sent to ' + coSigners.length + ' co-signatories');
